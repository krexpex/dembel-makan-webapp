...
// Фиксированные даты
const SERVICE_START = "2025-11-28T00:00:00"; // отсчёт начнётся в этот день
...
export default function App() {
  ...
  // НОВОЕ: флаг "до начала службы"
  const isPreStart = now < startTs;
  const startDateHuman = shortDate(SERVICE_START);
  ...
  useEffect(() => {
    if (isOver && !confettiDoneRef.current) {
      ...
      setTimeout(()=>confetti({
        particleCount: 250, spread: 85, startVelocity: 45, scalar: 1.1, ticks: 240, origin: { y: 0.25 }
      }), 900);
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-[#0f1514] to-[#0b1110] text-zinc-50">
      ...
      <div className={`fixed left-3 top-3 z-[60] transition-transform duration-250 ${burgerHidden?"-translate-y-14 opacity-0":"translate-y-0 opacity-100"}`}>
        <button onClick={()=>setMenuOpen(o=>!o)} className="glass-btn h-11 w-11 grid place-items-center rounded-2xl" aria-label="menu">
          <BurgerIcon/>
        </button>
        {menuOpen && (
          ...
          <section className="relative flex flex-col items-center justify-center rounded-3xl bg-zinc-900/60 backdrop-blur p-5 md:p-6 shadow-xl overflow-hidden">
            ...
            <div className={`mt-2 text-center ${entered?"appear-fade-up":""}`}>
              {isOver ? (
                ...
                <>
                  <h1 className="text-lg md:text-xl font-semibold text-zinc-300">Отсчёт начнётся</h1>
      ...
      {/* Островок — как у тебя, без изменений дизайна */}
...
/* ========= ОСТРОВОК — только иконки (без изменений) ========= */
function BottomIsland({ tab, onChange, dir }) {
  ...
  const centerOf = (btn) => {
    const cont = contRef.current; if (!cont || !btn?.current) return { x: 0, w: D };
  ...
  useLayoutEffect(() => {
    ...
    const recalc = () => {
      ...
      setBubble({ x, w: size }); setBlob({ x, w: size });
    };
  ...
  return (
    <nav className="fixed left-0 right-0 bottom-[calc(12px+env(safe-area-inset-bottom,0px))] z-[55] flex justify-center px-4 pointer-events-none">
      ...
      >
        {/* Пузырёк */}
        <div className={`ui-bubble ${mounted ? "bubble-mounted" : ""}`} style={{ "--x": `${bubble.x}px`, "--w": `${bubble.w}px` }}>
          <div className="ui-bubble-core" />
        </div>
        {/* Капля */}
        <div className={`ui-blob ${dir==="right"?"to-right":"to-left"} ${mounted?"blob-mounted":""}`} style={{ "--x": `${blob.x}px`, "--w": `${blob.w}px` }}>
          <div className="ui-blob-core" />
        </div>
        {/* Кнопки */}
        <div className="ui-row" style={{ height: 68 }}>
          <button ref={b1} className={`ui-btn ${tab==="timer"?"active":""}`}  aria-label="Timer"  onClick={()=>onChange("timer")}  style={{ "--d": `${54}px` }}><HelmetIcon/></button>
          <button ref={b2} className={`ui-btn ${tab==="id"?"active":""}`}      aria-label="ID"     onClick={()=>onChange("id")}     style={{ "--d": `${54}px` }}><IdCardIcon/></button>
          <button ref={b3} className={`ui-btn ${tab==="medals"?"active":""}`}  aria-label="Medal"  onClick={()=>onChange("medals")} style={{ "--d": `${54}px` }}><MedalIcon/></button>
        </div>
      </div>
    </nav>
  );
...
function shortDate(iso){
  const ts=toLocalTimestamp(iso); const d=new Date(ts);
